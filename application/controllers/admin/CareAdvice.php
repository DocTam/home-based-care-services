...
class CareAdvice extends App {
    ...
    function save() {
        ...
        if ($this->checksubmit()) {
            ...
            if ($advice_id > 0) {//修改
                $detail = K::M('care/careAdvice')->getone("advice_id={$advice_id}");
                if (empty($detail)) {
                ...
                K::M('care/careAdvice')->update($data, "advice_id={$advice_id}");
                $this->pagedata['data'] = array(
                    'Success' => true,
                    'Msg' => '保存成功!',
                    'advice_id' => $advice_id,
                ...
                K::M('OptLog')->addlog($advice_id, K::M('care/careAdvice')->table(), CONF::$LogType_edit, $data); //操作日志
                ...
                if ($advice_id = K::M('care/careAdvice')->add($data)) {

                    K::M('OptLog')->addlog($advice_id, K::M('care/careAdvice')->table(), CONF::$LogType_add, $data); //操作日志
    ...
    public function add() {
        ...
        $this->tmpl = 'care/careAdvice_edit.html';
    }
    ...
    public function delone() {
        ...
        if (K::M('care/careAdvice')->del("advice_id={$advice_id} {$this->other_where}")) {

            K::M('OptLog')->addlog($advice_id, K::M('care/careAdvice')->table(), CONF::$LogType_del, '删除'); //操作日志
