...
class Gulao extends App {
    ...
    public function index() {
        ...
        $pagenum = 1;
        if ($items = K::M('Gulao/Gulao')->items($filter, $order, $page, $limit, $count)) {
    ...
    function save() {
        ...
        if ($this->checksubmit()) {
            ...
            if ($gulao_id > 0) {//修改
                $detail = K::M('Gulao/Gulao')->getone("gulao_id={$gulao_id}");
                if (empty($detail)) {
                ...
                K::M('Gulao/Gulao')->update($data, "gulao_id={$gulao_id}");
                $this->pagedata['data'] = array(
                    'Success' => true,
                    'Msg' => '保存成功!',
                    'gulao_id' => $gulao_id,
                ...
                K::M('OptLog')->addlog($gulao_id, K::M('Gulao/Gulao')->table(), CONF::$LogType_edit, $data); //操作日志
                ...
                if ($gulao_id = K::M('Gulao/Gulao')->add($data)) {

                    K::M('OptLog')->addlog($gulao_id, K::M('Gulao/Gulao')->table(), CONF::$LogType_add, $data); //操作日志
    ...
    public function add() {
        ...
        $this->tmpl = 'Gulao/edit.html';
    }

    public function edit() {
        $gulao_id = intval($this->GP('id'));
    ...
    public function delone() {
        ...
        if (K::M('Gulao/Gulao')->del("gulao_id={$gulao_id} {$this->other_where}")) {

            K::M('OptLog')->addlog($gulao_id, K::M('Gulao/Gulao')->table(), CONF::$LogType_del, '删除'); //操作日志
