...
class Modelworker extends App {
    ...
    public function index() {
        ...
        $pagenum = 1;
        if ($items = K::M('Modelworker/Modelworker')->items($filter, $order, $page, $limit, $count)) {
        ...
        $this->tmpl = 'Modelworker/list.html';
    }
    ...
    function save() {
        ...
        if ($this->checksubmit()) {
            ...
            if ($worker_id > 0) {//修改
                $detail = K::M('Modelworker/Modelworker')->getone("worker_id={$worker_id}");
                if (empty($detail)) {
                ...
                K::M('Modelworker/Modelworker')->update($data, "worker_id={$worker_id}");
                $this->pagedata['data'] = array(
                    'Success' => true,
                    'Msg' => '保存成功!',
                    'worker_id' => $worker_id,
                ...
                K::M('OptLog')->addlog($worker_id, K::M('Modelworker/Modelworker')->table(), CONF::$LogType_edit, $data); //操作日志
                ...
                if ($worker_id = K::M('Modelworker/Modelworker')->add($data)) {

                    K::M('OptLog')->addlog($worker_id, K::M('Modelworker/Modelworker')->table(), CONF::$LogType_add, $data); //操作日志
    ...
    public function add() {
        ...
        $this->tmpl = 'Modelworker/edit.html';
    }
    ...
    public function delone() {
        ...
        if (K::M('Modelworker/Modelworker')->del("worker_id={$worker_id} {$this->other_where}")) {

            K::M('OptLog')->addlog($worker_id, K::M('Modelworker/Modelworker')->table(), CONF::$LogType_del, '删除'); //操作日志
