...
class VolunteerAdvice extends App {
    ...
    public function __construct() {
		...
		$this->code = K::M('worker/workerType')->getcode('volunteer');
        $this->pagedata['pagecode'] = 'volunteer';
    }
    ...
    public function index() {
        ...
        $pagenum = 1;
        if ($items = K::M('volunteer/volunteerAdvice')->items($filter, $order, $page, $limit, $count)) {
            $pagenum = ceil($count / $limit);
        }

        $names = K::M('worker/worker')->getnames("code='{$this->code}' {$this->other_where}");
        $this->pagedata['volunteers'] = $names;

        $this->pagedata['SO'] = $SO;
        $this->pagedata['items'] = $items;
        $this->pagedata['pager'] = $this->getPager($page, $count, $pagenum, $SO);

        $this->tmpl = 'volunteer/volunteerAdvice.html';
    }
    ...
    function save() {
        ...
        if ($this->checksubmit()) {
            ...
            if ($advice_id > 0) {//修改
                $detail = K::M('volunteer/volunteerAdvice')->getone("advice_id={$advice_id}");
                if (empty($detail)) {
                ...
                K::M('volunteer/volunteerAdvice')->update($data, "advice_id={$advice_id}");
                $this->pagedata['data'] = array(
                    'Success' => true,
                    'Msg' => '保存成功!',
                    'advice_id' => $advice_id,
                ...
                K::M('OptLog')->addlog($advice_id, K::M('volunteer/volunteerAdvice')->table(), CONF::$LogType_edit, $data); //操作日志
                ...
                if ($advice_id = K::M('volunteer/volunteerAdvice')->add($data)) {

                    K::M('OptLog')->addlog($advice_id, K::M('volunteer/volunteerAdvice')->table(), CONF::$LogType_add, $data); //操作日志
    ...
    public function add() {
        ...
        $this->tmpl = 'volunteer/volunteerAdvice_edit.html';
    }
    ...
    public function delone() {
        ...
        if (K::M('volunteer/volunteerAdvice')->del("advice_id={$advice_id} {$this->other_where}")) {

            K::M('OptLog')->addlog($advice_id, K::M('volunteer/volunteerAdvice')->table(), CONF::$LogType_del, '删除'); //操作日志
